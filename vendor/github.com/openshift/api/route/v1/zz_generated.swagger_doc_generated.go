...
var map_RouteHTTPHeaderActionUnion = map[string]string{
	...
	"set":  "set defines the HTTP header that should be set: added if it doesn't exist or replaced if it does. If this field is empty, no header is added or replaced.",
}
...
var map_RouteHTTPHeaderActions = map[string]string{
	...
	"response": "response is a list of HTTP response headers to set or delete. The actions either Set or Delete will be performed on the headers in sequence as defined in the list of response headers. A maximum of 20 response header actions may be configured. You can use this field to specify HTTP response headers that should be set or deleted when forwarding responses from your application to the client. Sample fetchers allowed are \"req.hdr\" and \"ssl_c_der\". Converters allowed are \"lower\" and \"base64\". Example header values: \"%[res.hdr(X-target),lower]\", \"%{+Q}[ssl_c_der,base64]\". Note: This field cannot be used if your route uses TLS passthrough. ",
	"request":  "request is a list of HTTP request headers to set or delete. The actions either Set or Delete will be performed on the headers in sequence as defined in the list of request headers. A maximum of 20 request header actions may be configured. You can use this field to specify HTTP request headers that should be set or deleted when forwarding connections from the client to your application. Sample fetchers allowed are \"req.hdr\" and \"ssl_c_der\". Converters allowed are \"lower\" and \"base64\". Example header values: \"%[req.hdr(X-target),lower]\", \"%{+Q}[ssl_c_der,base64]\". Note: This field cannot be used if your route uses TLS passthrough. ",
}
...
var map_RouteHTTPHeaders = map[string]string{
	...
	"actions": "actions specifies actions to take on headers. Note that this option only applies to cleartext HTTP connections and to secure HTTP connections for which the ingress controller terminates encryption (that is, edge-terminated or reencrypt connections).  Headers cannot be set or deleted for TLS passthrough connections. Setting HSTS header is supported via another mechanism in Ingress.Spec.RequiredHSTSPolicies. Any header configuration applied directly via a Route resource using this API\n will override header configuration for a header of the same name applied via\nspec.httpHeaders.actions on the IngressController or route annotation. The headers set via this API will not appear in access logs.  In addition, any headers set via this API on the route will overwrite headers set via spec.clientTLS,spec.httpHeaders.forwardedHeaderPolicy, spec.httpHeaders.uniqueId, and spec.httpHeaders.headerNameCaseAdjustments IngressController options. Setting \"cookie\" or \"set-cookie\" headers that conflict with OpenShift router's session cookies will result in undefined behavior. Note that the total size of all added headers *after* interpolating dynamic values must not exceed the value of spec.tuningOptions.headerBufferMaxRewriteBytes on the IngressController.",
}
