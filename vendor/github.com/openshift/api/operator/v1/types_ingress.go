...
type IngressControllerHTTPHeaders struct {
	...
	// The header settings made here can be overwritten by spec.httpHeaders or annotations
	// on Route objects.
...
type IngressControllerHTTPHeaderActions struct {
	...
	// deleted at the global level when forwarding requests to your application from the client, i.e. for all routes.
...
type IngressControllerHTTPHeader struct {
	...
	// +kubebuilder:validation:XValidation:rule="self.lowerAscii() != 'host'",message="host header may not be set or deleted via header actions"
	Name string `json:"name"`
